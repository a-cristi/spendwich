...
- Reports modes: Monthly · Yearly · Cash Flow · Compare · Custom range · All time. Cash Flow and Compare are new modes; the View section (By category/label + Expenses/Income tabs) is hidden for these two modes.
...
- Cash Flow chart: Chart.js mixed chart — two `'bar'` datasets (income/expenses) plus one `'line'` dataset (cumulative net, indigo, `tension: 0.3`). Single Y axis shared by bars and line. Range selector uses `.seg-group` with 6 mo / 12 mo / 24 mo buttons.
- Compare mode: always monthly for v1. Two period pickers (Period A / Period B) using `buildMonthPicker(label, value, onChange)` helper. Defaults to previous month vs current month (handles January edge case). `diffBreakdown(itemsA, itemsB, nameKey)` computes the diff — filters to expense items only, merges both sets by name, sorts by `abs(amtB)`. Delta column: `+N%` in red (higher spend), `−N%` in green (lower spend), `'new'` / `'gone'` when one side is zero. Grouped bar chart shows top 8 categories only; delta table shows all.
